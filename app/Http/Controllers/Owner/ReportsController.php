...
class ReportsController extends Controller
{
    ...
    public function index()
    {
        ...
        $fieldNetRevenue =
            DB::table('field_bookings')
                ...
                ->withTrashed() // Tambahan: untuk soft delete pada fields
        ...
        $rentalNetRevenue =
            DB::table('rental_bookings')
                ...
                ->withTrashed() // Tambahan: untuk soft delete pada rental_items
        ...
        $photographerNetRevenue =
            DB::table('photographer_bookings')
                ...
                ->withTrashed() // Tambahan: untuk soft delete pada photographers
        ...
        $productSalesNetRevenue =
            DB::table('product_sale_items')
                ...
                ->withTrashed() // Tambahan: untuk soft delete pada products
        ...
        $membershipNetRevenue =
            DB::table('membership_subscriptions')
                ...
                ->withTrashed() // Tambahan: untuk soft delete pada memberships
    ...
    public function revenueReport(Request $request)
    {
        ...
        $fieldBookingRevenue = DB::table('field_bookings')
            ...
            ->where('field_bookings.is_membership', 0)
            ->whereBetween('field_bookings.created_at', [$startDateTime, $endDateTime])
            ->sum('field_bookings.total_price');
        ...
        $rentalRevenue = DB::table('rental_bookings')
            ...
            ->where('rental_bookings.is_membership', 0)
            ->whereBetween('rental_bookings.created_at', [$startDateTime, $endDateTime])
            ->sum('rental_bookings.total_price');
        ...
        $photographerRevenue = DB::table('photographer_bookings')
            ...
            ->where('photographer_bookings.is_membership', 0)
            ->whereBetween('photographer_bookings.created_at', [$startDateTime, $endDateTime])
            ->sum('photographer_bookings.price');
    ...
    public function fieldRevenueReport(Request $request)
    {
        ...
        $fields = Field::withTrashed()->all(); // Sudah benar menggunakan withTrashed()
        ...
        $dailyRevenue = DB::table('field_bookings')
            ->join('fields', 'field_bookings.field_id', '=', 'fields.id')
            ->withTrashed() // Sudah benar menggunakan withTrashed()
            ->leftJoin('payments', 'field_bookings.payment_id', '=', 'payments.id')
            ->whereBetween('field_bookings.start_time', [$startDateTime, $endDateTime])
            ->where('field_bookings.status', 'confirmed')
            ->where('field_bookings.is_membership', 0)
    ...
    public function transactionHistory(Request $request)
    {
        ...
        $query = Payment::with([
            'customer',
            'fieldBookings.field' => function ($query) {
                $query->withTrashed(); // Tambahan: untuk soft delete pada fields
            },
            'rentalBookings.rentalItem' => function ($query) {
                $query->withTrashed(); // Tambahan: untuk soft delete pada rental_items
            },
            'photographerBookings.photographer' => function ($query) {
                $query->withTrashed(); // Tambahan: untuk soft delete pada photographers
            },
            'productItems.product' => function ($query) {
                $query->withTrashed(); // Tambahan: untuk soft delete pada products
            },
        ])
            ->where(function ($q) {
        ...
        if ($search) {
            $query->where(function ($q) use ($search) {
                $q->where('order_id', 'like', "%{$search}%")->orWhereHas('customer', function ($customerQuery) use ($search) {
                    $customerQuery->where('name', 'like', "%{$search}%")->orWhere('phone_number', 'like', "%{$search}%");
                });
            });
        }
        ...
        return view('owner.reports.transactions', compact('transactions', 'totalRevenue', 'revenueByPaymentType', 'chartData', 'startDate', 'endDate', 'search', 'paymentType'));
    }
    ...
    public function productSalesRevenueReport(Request $request)
    {
        ...
        $productRevenueByItem = ProductSaleItem::with([
            'product' => function ($query) {
                $query->withTrashed(); // Sudah benar menggunakan withTrashed()
            },
        ])
            ->join('products', 'product_sale_items.product_id', '=', 'products.id')
            ->withTrashed() // Sudah benar menggunakan withTrashed()
        ...
        $topSellingProducts = ProductSaleItem::with([
            'product' => function ($query) {
                $query->withTrashed(); // Sudah benar menggunakan withTrashed()
            },
        ])
            ...
            ->whereBetween('payments.created_at', [$startDateTime, $endDateTime])
            ->select('products.id', 'products.name', 'products.category', 'products.price as unit_price', DB::raw('SUM(product_sale_items.quantity) as total_quantity'), DB::raw('SUM(product_sale_items.price * product_sale_items.quantity) as total_revenue'))
            ->groupBy('products.id', 'products.name', 'products.category', 'products.price')
            ->orderBy('total_quantity', 'desc')
            ->take(5)
            ->get();

        return view('owner.reports.product-sales-revenue', compact('productRevenueByItem', 'totalProductNetRevenue', 'productRevenueByDay', 'productRevenueByCategory', 'topSellingProducts', 'startDate', 'endDate'));
    }
}

...
@section('title', 'Edit Paket Fotografer')
@section('breadcrumb', 'Edit Paket Fotografer')
@section('header-title', 'Edit Paket Fotografer')
...
<div class="row">
    <div class="col-lg-12">
        <div class="card">
            <div class="card-body">
                <h4 class="mt-0 header-title">Form Edit Paket Fotografer</h4>

                <form action="{{ route('admin.photo-packages.update', $photographer->id) }}" method="POST" enctype="multipart/form-data" id="photographerForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group mb-3">
                                <label>Fotografer <span class="text-danger">*</span></label>
                                <select name="user_id" class="form-control @error('user_id') is-invalid @enderror" required>
                                    <option value="">-- Pilih Fotografer --</option>
                                    @foreach($photographers as $photo)
                                        <option value="{{ $photo->id }}" {{ old('user_id', $photographer->user_id) == $photo->id ? 'selected' : '' }}>
                                            {{ $photo->name }} ({{ $photo->email }})
                                        </option>
                                    @endforeach
                                </select>
                                <small class="form-text text-muted">Pilih fotografer yang akan menggunakan paket ini</small>
                                @error('user_id')
                            ...
                            <div class="form-group mb-3">
                                ...
                                <small class="form-text text-muted">Contoh: Paket Favorite, Paket Plus, Paket Exclusive</small>
                                @error('name')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="form-group mb-3">
                                ...
                                <textarea name="description" class="form-control @error('description') is-invalid @enderror"
                                          rows="4" required>{{ old('description', $photographer->description) }}</textarea>
                            ...
                            <div class="form-group mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" name="price" class="form-control @error('price') is-invalid @enderror"
                                           value="{{ old('price', $photographer->price) }}" required>
                                </div>
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select name="package_type" class="form-control @error('package_type') is-invalid @enderror" required>
                                    <option value="">-- Pilih Jenis Paket --</option>
                                    <option value="basic" {{ old('package_type', $photographer->package_type) == 'basic' ? 'selected' : '' }}>Basic</option>
                                    <option value="favorite" {{ old('package_type', $photographer->package_type) == 'favorite' ? 'selected' : '' }}>Favorite</option>
                                    <option value="plus" {{ old('package_type', $photographer->package_type) == 'plus' ? 'selected' : '' }}>Plus</option>
                                    <option value="exclusive" {{ old('package_type', $photographer->package_type) == 'exclusive' ? 'selected' : '' }}>Exclusive</option>
                                </select>
                                @error('package_type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="col-md-6">
                            <div class="form-group mb-3">
                                ...
                                <input type="number" name="duration" class="form-control @error('duration') is-invalid @enderror"
                                       value="{{ old('duration', $photographer->duration) }}" min="1" required>
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select name="status" class="form-control @error('status') is-invalid @enderror" required>
                                    <option value="active" {{ old('status', $photographer->status) == 'active' ? 'selected' : '' }}>Aktif</option>
                                    <option value="inactive" {{ old('status', $photographer->status) == 'inactive' ? 'selected' : '' }}>Tidak Aktif</option>
                                </select>
                                @error('status')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="form-group mb-3">
                                ...
                                <div id="featuresContainer">
                                    @php
                                    // Check if old('features') exists and is an array
                                    if (old('features')) {
                                        $features = old('features');
                                    } else {
                                        // Only decode if it's a string
                                        $features = is_string($photographer->features)
                                            ? json_decode($photographer->features)
                                    ...
                                    @if(is_array($features) && count($features) > 0)
                                        @foreach($features as $index => $feature)
                                            <div class="input-group mb-2 feature-item">
                                                <input type="text" name="features[]" class="form-control @if($errors->has('features.'.$index)) is-invalid @endif"
                                                ...
                                                @if($errors->has('features.'.$index))
                                                    <div class="invalid-feedback">{{ $errors->first('features.'.$index) }}</div>
                                                @endif
                                            </div>
                                        @endforeach
                                    @else
                        ...
                        <div class="form-group mb-3">
                            ...
                            <small class="form-text text-muted">Pilih lapangan jika paket fotografer spesifik untuk lapangan tertentu</small>
                            @error('field_id')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>

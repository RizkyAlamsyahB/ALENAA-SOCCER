                                ...
                                @foreach($allBookings['rental_bookings'] as $booking)
                                    <div class="detail-item">
                                        <div class="row align-items-center">
                                            <div class="col-md-8 col-sm-7 mb-2 mb-md-0">
                                                <div class="item-details">
                                                    <h5 class="item-title fw-bold mb-1">{{ $booking->rentalItem->name }}</h5>
                                                    <div class="item-category mb-2">
                                                        <span class="type-badge">Penyewaan Peralatan</span>
                                                    </div>
                                                    <div class="item-info">
            ...
            snap.pay('{{ $snap_token }}', {
                onSuccess: function(result) {
                    @if(isset($is_continue_payment) && $is_continue_payment)
                        window.location.href = '{{ route("user.payment.success") }}?order_id={{ $original_order_id }}&payment_id={{ $payment_id }}';
                    @else
                ...
                onPending: function(result) {
                    @if(isset($is_continue_payment) && $is_continue_payment)
                        window.location.href = '{{ route("user.payment.unfinish") }}?order_id={{ $original_order_id }}&payment_id={{ $payment_id }}';
                    @else
                ...
                onError: function(result) {
                    @if(isset($is_continue_payment) && $is_continue_payment)
                        window.location.href = '{{ route("user.payment.error") }}?order_id={{ $original_order_id }}&payment_id={{ $payment_id }}';
                    @else
